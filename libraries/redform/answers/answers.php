...
class RdfAnswers
{
	...
	public function sendSubmitterNotification()
	{
		...
		foreach ($emails as $key => $submitter_email)
		{
			...
			if (JMailHelper::isEmailAddress($submitter_email))
			{
				...
				/* Start replace redSHOP coupon tag */
				JPluginHelper::importPlugin('redshop_coupon');
				$dispatcher = JDispatcher::getInstance();
				$submission_body = $dispatcher->trigger('replaceCoupon', array($submission_body, $key, $emails));
				$submission_body = $submission_body[0];
				/* End replace redSHOP coupon tag */
