	...
	$.fn.dragndrop = function(options) {
		...
		function createStatusbar(obj) {
			...
			this.setFileNameSize = function(name, size) {
				var fieldCode = mainObject.attr('target');
				var sizeStr = "";
				...
				$('#jform_fields_file_names_' + fieldCode).val(name);
				$('#jform_fields_images_' + fieldCode + '_media_preview').html('');
				$('#jform_fields_media_' + fieldCode).val('');
				$('#img-crop-' + fieldCode).remove();
				$('#' + fieldCode + '_current').remove();
		...
		function sendFileToServer(formData, status, parentDiv) {
			...
			var jqXHR = jQuery.ajax({
				...
				url: uploadURL + '&uploadType=' + uploadType + '&uploadTarget=' + uploadTarget + '&fieldType=' + fieldType,
				type: "POST",
				contentType: false,
				processData: false,
				cache: false,
		...
		function responseHandlerProcess(data, status, parentDiv) {
			...
			if (uploadType == 'file') {
				$('#jform_fields_dragndrop_upload' + uid).val(data.trim());
			}

			if (uploadType == 'gallery') {
				$('#jform_fields_dragndrop_upload' + uid).val($('#jform_fields_dragndrop_upload' + uid).val() + ',' + data.trim());
				status.deleteFile = $("<div class='deleteFile'><a class='btn btn-danger dragndrop-delete-file'><i class='icon-remove'></i>" + Joomla.JText._("COM_REDITEM_UPLOAD_DELETE_FILE") + "</a></div>").appendTo(status.statusbar);
				...
				status.deleteFile.click(function() {
					$('#jform_fields_dragndrop_upload' + uid).val($('#jform_fields_dragndrop_upload' + uid).val().replace(',' + data.trim(), ''));
			...
			if (uploadType == 'image') {
				...
				if (!$('#img_preview_' + uploadTarget).length) {
					imgPreview.prepend('<div class="pull-left"><div class="img-preview-container" style="position:relative;"><img id="img_preview_' + uploadTarget + '" src="" class="img-polaroid" style="max-width: 300px; max-height: 300px; margin-right: 20px" /></div></div>');
				}

				$('#img_preview_' + uploadTarget).attr('src', img_preview_path + data.trim());

				status.deleteFile.click(function() {
					$('#jform_fields_dragndrop_upload' + uid).val('');
		...
		if (isEventSupported('dragenter') && isEventSupported('dragover') && isEventSupported('drop')) {
			...
			mainObject.after('<input type="hidden" id="jform_fields_dragndrop_upload' + uid + '" name="jform[fields][dragndrop][' + uploadTarget + '][]" value="" />');
			...
			if (!isFileAPIEnabled()) {
				$('<input>').attr('type', 'file')
					...
					.appendTo($('#reditem_dragselect_' + uid))
					.ajaxfileupload({
						action: options.url + '&uploadType=' + uploadType + '&uploadTarget=' + uploadTarget + '&fieldType=' + fieldType,
						params: {
							extra: 'info'
						},
