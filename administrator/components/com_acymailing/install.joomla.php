...
class acymailingInstall{
	...
	`published` tinyint NOT NULL DEFAULT '0',`ordering` smallint unsigned NULL DEFAULT '0',PRIMARY KEY (`action_id`)) /*!40100 DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci*/;");
		...
		if(version_compare($this->fromVersion, '5.9.3', '<')){
			if(!ACYMAILING_J16){
				$this->updateQuery("UPDATE #__plugins SET `ordering` = 26 WHERE `element` = 'urltracker' AND `folder` = 'acymailing'");
			}else{
				$this->updateQuery("UPDATE #__extensions SET `ordering` = 26 WHERE `element` = 'urltracker' AND `folder` = 'acymailing'");
			}
		}
	}
