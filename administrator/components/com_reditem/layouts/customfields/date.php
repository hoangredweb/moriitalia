...
$phpFormat      = ReditemHelperCustomfield::convertDateFormat($format, 'php');
$momentFormat   = str_replace('yy', 'yyyy', $format);
$momentFormat   = strtoupper($momentFormat);
...
<script type="text/javascript">
	(function($){
		$(document).ready(function(){
			$('#cform_date_<?php echo $fieldcode; ?>').datepicker({
				dateFormat: 'yy-mm-dd',
				<?php if ($disablePast) : ?>
				...
				altField: "#cform_date_<?php echo $fieldcode; ?>",
				...
				var value = $('#cform_date_<?php echo $fieldcode; ?>_value').val();
			...
			$('#cform_date_<?php echo $fieldcode; ?>_icon').click(function(event){
				event.preventDefault();
				$('#cform_date_<?php echo $fieldcode; ?>').datepicker('show');
			});

			<?php if ($showTimePicker) : ?>
			$('#cform_date_<?php echo $fieldcode; ?>_time').timepicker({
				showPeriodLabels: false,
				minutes: {
					interval: 5
				},
				showOn: 'both',
				button: $('#cform_date_<?php echo $fieldcode; ?>_time_icon'),
		...
		function reditemCustomFieldDateProcess_<?php echo $fieldcode; ?>(dateValue)
		{
			...
			var timeValueFromInput = $('#cform_date_<?php echo $fieldcode; ?>_time').val();
			if (timeValueFromInput != '') timeValue = timeValueFromInput;
			<?php endif; ?>

			$('#cform_date_<?php echo $fieldcode; ?>_value').val(dateValue + ' ' + timeValue);
		}
	})(jQuery);
</script>

<div class="reditem_customfield_date" id="cform_date_<?php echo $fieldcode; ?>_wrapper">
	<div class="input-append">
		<input  id="cform_date_<?php echo $fieldcode; ?>"
				type="text" class="input <?php echo $class ?>"
				value="<?php echo $dateFValue;?>"
				placeholder="<?php echo $format;?>"
		>
		<span class="add-on" id="cform_date_<?php echo $fieldcode; ?>_icon">
			<i class="icon-calendar"></i>
		</span>
	</div>
	<?php if ($showTimePicker) : ?>
	<div class="input-append">
		<input id="cform_date_<?php echo $fieldcode; ?>_time" type="text" class="input-small <?php echo $class ?>" value="<?php echo $timeValue; ?>">
		<span class="add-on" id="cform_date_<?php echo $fieldcode; ?>_time_icon">
			<i class="icon-time"></i>
		</span>
	</div>
	<?php endif; ?>
	<input type="hidden" id="<?php echo $fieldcode; ?>_tmp_date" value="<?php echo $dateValue;?>" />
	<input type="hidden" name="cform[date][<?php echo $fieldcode; ?>]" id="cform_date_<?php echo $fieldcode; ?>_value" value="<?php echo $value; ?>" />
</div>
