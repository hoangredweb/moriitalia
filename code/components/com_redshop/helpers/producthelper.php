...
class productHelper
{
	...
	public function replaceWeddingButton($product_id = 0, $data_add = "")
	{
		$myWeddingLink = JUri::root()
			. 'index.php?tmpl=component&option=com_redshop&view=wedding_list&task=createsave&tmpl=component&product_id='
			. $product_id;
		$weddingExist = '';
		$weddingIcon = "<i class=\"icon-heart-empty\"></i> ";
		$checkWedding = $this->checkWeddingExist($product_id);

		if ($checkWedding > 0)
		{
			$weddingExist = ' active';
			$weddingIcon = "<i class=\"icon-heart\"></i> ";
		}

		$weddingLink = JText::_("COM_REDSHOP_ADD_TO_WEDDING");
		$weddingPrefix = "<a class=\"wedding-list" . $weddingExist . "\" href=\"" . $myWeddingLink . "\" >".$weddingIcon;
		$weddingSuffix = '</a>';

		$weddingLink = $weddingPrefix . $weddingLink . $weddingSuffix;

		$data_add = str_replace('{wedding_link}', $weddingLink, $data_add);

		return $data_add;
	}

	public function checkWeddingExist($productId = 0)
	{
		$user = JFactory::getUser();
		$db = JFactory::getDbo();
		$query = $db->getQuery(true)
			->select('COUNT(*)')
			->from($db->qn('#__redshop_wedding_list', 'w'))
			->where($db->qn('w.product_id') . ' = ' . $db->q((int) $productId))
	...
	public function getMedia($type, $id)
	{
		...
		$query = $db->getQuery(true)
			...
			->where($db->qn('section_id') . ' = ' . $db->q((int) $id))
			->where($db->qn('media_section') . ' = ' . $db->q($type));
