...
class plgContentJFBCContent extends JPlugin
{
    ...
    private function getSocialItemViewPosition($article, $infix = 'like')
    {
        ...
        if ($this->view == 'article' && $article->id != null)
            $returnValue = $this->configModel->getSetting('social_'.$infix.'_article_view');
        else if ($this->view == 'frontpage' || $this->view == 'featured')
            $returnValue = $this->configModel->getSetting('social_'.$infix.'_frontpage_view');
        else if ($this->view == 'category' && $article->catid != null)
            $returnValue = $this->configModel->getSetting('social_'.$infix.'_category_view');
    ...
    private function getSocialK2ItemViewPosition($article, $infix = 'like')
    {
        ...
        if ($this->view == 'item' && $article->id != null)
            $returnValue = $this->configModel->getSetting('social_k2_'.$infix.'_item_view');
        else if ($this->view == 'itemlist')
        {
            if ($this->isK2Layout($layout, $task, 'category')
                ...
                $returnValue = $this->configModel->getSetting('social_k2_'.$infix.'_category_view');
            else if ($this->isK2Layout($layout, $task, 'generic') || $this->isK2Layout($layout, $task, 'tag'))
                $returnValue = $this->configModel->getSetting('social_k2_'.$infix.'_tag_view');
            else if ($this->isK2Layout($layout, $task, 'user') && $this->app->input->getInt('id', 0))
                $returnValue = $this->configModel->getSetting('social_k2_'.$infix.'_userpage_view');
        } else if ($this->view == 'latest')
            $returnValue = $this->configModel->getSetting('social_k2_'.$infix.'_latest_view');
    ...
    {
        $articleIncludeIds =  $this->configModel->getSetting('social_'.$infix.'_article_include_ids');
        $articleExcludeIds =  $this->configModel->getSetting('social_'.$infix.'_article_exclude_ids');
        $catIncludeType =  $this->configModel->getSetting('social_'.$infix.'_cat_include_type');
        $catIds =  $this->configModel->getSetting('social_'.$infix.'_cat_ids');
    ...
    {
        $articleIncludeIds = $this->configModel->getSetting('social_k2_'.$infix.'_item_include_ids');
        $articleExcludeIds = $this->configModel->getSetting('social_k2_'.$infix.'_item_exclude_ids');
        $catIncludeType = $this->configModel->getSetting('social_k2_'.$infix.'_cat_include_type');
        $catIds = $this->configModel->getSetting('social_k2_'.$infix.'_cat_ids');
