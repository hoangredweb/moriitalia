...
$model->setProductLimit(9);
$limit = $model->getProductLimit();

$app = JFactory::getApplication();
$router = $app->getRouter();
$uri = new JURI('index.php?option=com_redshop&view=manufacturers&layout=products&mid=' . $manufacturer->id . '&Itemid=' . $Itemid . '&limit=' . $limit . '&order_by=' . $order_by_select . '&filter_by=' . $filter_by_select);
...
$template_desc = str_replace("{total_product}", JText::sprintf('COM_REDSHOP_TOTAL_PRODUCT_COUNT', count($manufacturer_products) ), $template_desc);
...
if ($template_middle != "")
{
	...
	for ($i = 0, $in = count($manufacturer_products); $i < $in; $i++)
	{
		...
		if (strstr($cart_mdata, "{category_heading_start}") && strstr($cart_mdata, "{category_heading_end}"))
		{
			...
			//var_dump($manufacturer_products[$i]);die;
...
if (strstr($template_desc, "{manufacturer_image}"))
{
	...
	$thum_image  = "";
	$media_image = $producthelper->getAdditionMediaImage($manufacturer->id, "manufacturer");
	$m           = 0;
...
//	{
		...
		&& file_exists(REDSHOP_FRONT_IMAGES_RELPATH . "manufacturer/" . $media_image[$m]->media_name))
	{
		...
		$altText = $producthelper->getAltText('manufacturer', $manufacturer->id);
...
$manufacturer_extra_fields = $extra_field->list_all_field_display(10, $manufacturer->id);
...
{
	$orderby_form = "<form name='orderby_form' action='' method='post'>" . $this->lists['order_select'];
...
if (strstr($template_desc, '{order_by_lbl}'))
{
	$orderby_form = JText::_('COM_REDSHOP_SELECT_ORDER_BY');
	$template_desc = str_replace("{order_by_lbl}", $orderby_form, $template_desc);
}
