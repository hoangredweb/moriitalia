...
$template_desc = str_replace('{person_information}', JText::_('COM_REDSHOP_ACCOUNT_PERSON_INFORMATION'), $template_desc);
...
if ( $user->username != "" )
{
	$customer_username_lbl = JText::_('COM_REDSHOP_CUSTOMER_USERNAME');
...
{
	$customer_lastname_lbl = JText::_('COM_REDSHOP_CUSTOMER_LASTNAME');
...
if ($this->userdata->firstname != "")
{
	$customer_first_lbl = JText::_('COM_REDSHOP_CUSTOMER_FIRSTNAME');
	$customer_firstname     = $this->userdata->firstname;
}

$template_desc = str_replace('{firstname_lbl}', $customer_first_lbl, $template_desc);
...
if (!empty($this->userdata->point))
{
	$customer_point_lbl = JText::_('COM_REDSHOP_POINT');
	$customer_point     = $this->userdata->point;
}
else
{
	$customer_point_lbl = JText::_('COM_REDSHOP_POINT');
...
$template_desc = str_replace('{point_lbl}', $customer_point_lbl, $template_desc);
$template_desc = str_replace('{point}', $customer_point, $template_desc);
...
if ( $cart['idx'] != 0 ) {
	$shopping_link = "index.php?option=com_redshop&view=checkout&Itemid=".$Itemid;
}else{
	$shopping_link = "index.php";
}
$continue_shopping_link = '<a class="btn btn-primary" href="'.$shopping_link.'">' . JText::_('COM_REDSHOP_ACCOUNT_CONTINUE_SHOPPING') . '</a>';
$template_desc     = str_replace('{continue_shopping}', $continue_shopping_link, $template_desc);
