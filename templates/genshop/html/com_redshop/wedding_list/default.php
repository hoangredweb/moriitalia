...
$pagetitle = JText::_('COM_REDSHOP_MY_WEDDINGLIST');
...
$userfieldArr = $returnArr[1];
	...
	echo "<div class='mod_redshop_wishlist'>";
		...
		{
			echo "<div>" . JText::_('COM_REDSHOP_NO_PRODUCTS_IN_WEDDINGLIST') . "</div>";
		}
		else
		{
...
function display_products($rows)
{
	...
	else
	{
		...
		for ($i = 0, $in = count($rows); $i < $in; $i++)
		{
			...
			$regdellink     = JURI::root() . "index.php?option=com_redshop&view=wedding_list&product_id=" . $row->product_id . "&task=delete";
