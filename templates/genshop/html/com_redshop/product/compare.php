...
$template = str_replace('{product_name_lbl}', JText::_('COM_REDSHOP_QUESTION_PRODUCT_NAME'), $template);
$template = str_replace('{product_image_lbl}', '', $template);
$template = str_replace('{manufacturer_name_lbl}', JText::_('COM_REDSHOP_MANUFACTURER'), $template);
$template = str_replace('{product_price_lbl}', JText::_('COM_REDSHOP_PRODUCT_PRICE'), $template);
$template = str_replace('{product_desc_lbl}', JText::_('COM_REDSHOP_DESCRIPTION'), $template);
$template = str_replace('{product_category_lbl}', JText::_('COM_REDSHOP_CATEGORY'), $template);
...
if ($total > 0)
{
	...
	foreach ($list as $data)
	{
		...
		$prod_sdesc = '<div class="sdesc">'. JHTML::_('string.truncate', strip_tags($product->product_desc), 100) .'</div>';
		$prod_fdesc = '<div class="fdesc">'. $product->product_desc .'</div>';
		$prod_btndesc = '<a href="#" class="compare_readmore">'.JText::_("COM_REDSHOP_READ_MORE").'</a>';
		$prod_btndesc .= '<a href="#" class="compare_readless">'.JText::_("COM_REDSHOP_READ_LESS").'</a>';
		$prod_desc = '<div class="desc_wrapper">'.$prod_sdesc.$prod_fdesc.$prod_btndesc.'</div>';
		$template = str_replace('{product_desc}', $exp_div . $prod_desc . $div_end . $td_end . $td_start . "{product_desc}", $template);
		...
		$link_remove = JUri::root() . 'index.php?option=com_redshop&view=product&task=removecompare&layout=compare&pid=' . $product->product_id . '&cid=' . $data['item']->categoryId . '&Itemid=' . $this->itemId . '&tmpl=component';
