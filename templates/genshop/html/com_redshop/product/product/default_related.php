...
if (count($relptemplate) > 0)
{
	...
	for ($r = 0, $rn = count($related_product); $r < $rn; $r++)
	{
		...
		if (strstr($related_template_data, "{product_id}"))
		{
			$related_template_data = str_replace("{product_id}", $related_product[$r]->product_id, $related_template_data);
		}
		...
		$wishlistLink = "<div class=\"wishlist\">" . $producthelper->replaceWishlistButton($related_product[$r]->product_id, '{wishlist_link}') ."</div>";
