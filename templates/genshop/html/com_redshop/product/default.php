...
JHtml::stylesheet(Juri::base() . 'media/com_reditem/css/colorbox.min.css');
...
if (strstr($template_desc, "{manufacturer_image}"))
{
	...
	if (!empty($media_image[$m]->media_name) && $media_image[$m]->media_name && file_exists(REDSHOP_FRONT_IMAGES_RELPATH . "manufacturer/" . $media_image[$m]->media_name))
	{
...
if($this->data->manufacturer_id == 0){
	$manuUrl = JRoute::_('index.php?option=com_redshop&Itemid=945&lang=vi&view=manufacturers');
}
else{
	$manuUrl           = JRoute::_(
...
// Replace weddingbutton
$template_desc = $producthelper->replaceWeddingButton($this->data->product_id, $template_desc);
