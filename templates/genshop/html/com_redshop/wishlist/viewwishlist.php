...
?>
<div class="wish_list_title"><h1><?php echo JText::_('COM_REDSHOP_VIEW_WISHLIST');?></h1></div>
<p class="wislist_subdesc"><?php echo JText::_('COM_REDSHOP_SUBDESC_WISHLIST');?></p>
<?php
if (!$user->id)
{
	$rows = $this->wish_session;

	echo "<div class='com_redshop_wishlist'>";
...
else
{
	...
	if (count($this->wish_session) > 0)
	{
		...
		echo "<br />";
		$mywishlist_link = JRoute::_("index.php?tmpl=component&option=com_redshop&view=wishlist&task=addtowishlist&tmpl=component");
	...
	if (count($wishlists) > 0)
	{
		...
		// Send mail link
		echo "<table>";
		echo "<tr><th colspan='4'>".JText::_('COM_REDSHOP_NAME_WISHLIST')."</th></tr>";
		foreach ($wishlists as $wishlist)
		{
			$wishlist_link = JRoute::_("index.php?view=account&layout=mywishlist&wishlist_id=" . $wishlist->wishlist_id . "&wishlist_name=".$wishlist->wishlist_name."&option=com_redshop&Itemid=" . $Itemid);
		...
		$wishlist_addbtn = JRoute::_("index.php?tmpl=component&option=com_redshop&view=wishlist&task=addtowishlist&tmpl=component");
		echo "<tr class='addbtn'><td colspan='4'>";
		echo "<div style=\"clear:both;\" ><a class=\"redcolorproductimg\" href=\"" . $wishlist_addbtn . "\"  ><input type='button'  value='" . JText::_('COM_REDSHOP_SAVE_WISHLIST') . "'></a></div>";
		echo "</td></tr>";
		echo "</table>";
	}
