...
else
{
	...
	$checkout .= '<div class="checkout_button">';
...
if (!empty($user->id))
{
	if (Redshop::getConfig()->get('USER_POINT') == 1)
	{
		if (!empty($info->point))
		{
			$userPointText = JText::sprintf('COM_REDSHOP_CURRENT_POINT', $info->point);
			$userPointLbl = JText::_('COM_REDSHOP_POINT');
			$userPoint .= '<div class="update_cart"><form action="index.php?option=com_redshop&view=cart" name="user_point_form" method="POST" >';
			...
			$userPoint .= '<input type="hidden" name="task" value="userPoint"><input type="hidden" name="Itemid" value="' . $Itemid . '">';
			$userPoint .= '</form></div>';
		}
	}
}

$cart_data = str_replace("{user_point_text}",  $userPointText, $cart_data);
$cart_data = str_replace("{user_point_lbl}",  $userPointLbl, $cart_data);
$cart_data = str_replace("{user_point}", $userPoint, $cart_data);
