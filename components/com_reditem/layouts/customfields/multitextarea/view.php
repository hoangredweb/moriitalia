...
<script type="text/javascript">
	...
	function textareaUpdate()
	{
		...
		jQuery('#cform_multitextarea_<?php echo $fieldcode; ?>').val('');

		<?php if (strpos($attributes, 'required')) : ?>
		if (texts.length > 0)
		{
			jQuery('#cform_multitextarea_<?php echo $fieldcode; ?>').val(JSON.stringify(texts));
		}
		<?php else : ?>
		jQuery('#cform_multitextarea_<?php echo $fieldcode; ?>').val(JSON.stringify(texts));
		<?php endif;?>
...
<div class="reditem_customfield_multitextarea" <?php echo $attributes; ?>>
	...
	<input type="hidden" name="cform[multitextarea][<?php echo $fieldcode; ?>]" id="cform_multitextarea_<?php echo $fieldcode; ?>" value='<?php echo $value; ?>' <?php echo $attributes; ?>/>
</div>
