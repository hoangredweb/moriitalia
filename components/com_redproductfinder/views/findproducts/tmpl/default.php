...
$listCat = $formHelper->getParentCategory();
...
$template_desc = str_replace("{total_product}", $model->getTotal(), $template_desc);
...
if (strstr($template_desc, "{product_loop_start}") && strstr($template_desc, "{product_loop_end}"))
{
	...
	{
		$template_desc .= JText::_('COM_REDPRODUCTFINDER_NO_ITEM');
	}
	// Loop product lists
	foreach ($this->products as $key => $pd)
	{
		...
		$thum_image = "<span class='productImageWrap' id='productImageWrapID_" . $product->product_id . "'>" . $producthelper->getProductImage($product->product_id, $link, $pw_thumb, $ph_thumb, 2, 0) . "</span>";
		...
		************************************/
		...
		if ($showCart == 0)
		{
			...
			{
				$data_add = str_replace("{form_addtocart:" . $cartTemplate[$i]->template_name . "}", "", $data_add);
			}
		}
	...
	$linkOrderBy = JRoute::_("index.php?option=com_redproductfinder&view=findproducts&cid=" . $cid);

	$checkCat = 0;

	if (!empty($this->data['category']))
	{
		$checkCat = $this->data['category'];
	}
	...
	$categories = "";
	$categories = "<form name='categories_form' action='" . $linkOrderBy . "' method='post' >";
	$categories .= "<select name='category' id='categories' class='inputbox' onchange='this.form.submit();'>";

	foreach ($listCat as $cat)
	{
		if ($cat->category_id == $checkCat)
		{
			$select = 'selected';
		}
		else
		...
		$categories .= "<option " . $select . " value='" . $cat->category_id . "'>" . $cat->category_name . "</option>";
	}

	$categories .= "</select>";
	$categories .= "<input type='hidden' name='view' value='findproducts'>
						<input type='hidden' name='limitstart' value='0'>
						<input type='hidden' name='jsondata' value='" . $this->json . "'>
						<input type='hidden' name='Itemid' value='" . $Itemid . "'>";
	$categories .= "</form>";
	...
	if (strstr($template_desc, '{categories}'))
	{
		$template_desc = str_replace("{categories}", $categories, $template_desc);
	}

	if (strstr($template_desc, "{pagination}"))
	{
		$pagination = $model->getPagination();
		$paginationForm = "<form action='index.php?option=com_redproductfinder&view=findproducts&cid=" . $cid . "' method='post' name='adminForm' id='adminForm'>
				<input type='hidden' name='view' value='findproducts'>
				<input type='hidden' name='order_by' value='" . $orderby . "'>
				<input type='hidden' name='Itemid' value='" . $Itemid . "'>
				<input type='hidden' name='limit' value='" . $limit . "'>
				<input type='hidden' name='jsondata' value='" . $this->json . "'>"
				. $pagination->getListFooter() . "</form>";
		$template_desc = str_replace("{pagination}", $paginationForm, $template_desc);
	}
	...
	if (strstr($template_desc, "{product_display_limit}"))
	{
		...
		{
			$limitBox = "<form action='index.php?option=com_redproductfinder&view=findproducts&cid=" . $cid . "' method='post'>
				<input type='hidden' name='view' value='findproducts'>
				<input type='hidden' name='order_by' value='" . $orderby . "'>
				<input type='hidden' name='Itemid' value='" . $Itemid . "'>
				<input type='hidden' name='jsondata' value='" . $this->json . "'>"
